...
#ponizej ladujemy sobie plik z memami
    ...
    if '!spam' in testowystr:
        ...
        if komenda.lower() == '!spam' :
            for i in range(0,int(liczba)):
                ...
                de_cat_spam ='C:/programowanko/memowo/'
    ...
    if ('linux' in testowystr) or ('ubuntu' in testowystr) or ('linuks' in testowystr):
        ...
        await message.channel.send(file=discord.File('E:\specjal\palpatine-emperor.gif'))
        time.sleep(4)
...
async def daty_godziny ():
    ...
    while not client.is_closed():
        ...
        if (int(godz) == 21) and (int(minuta) == 37) and papa == 0:
            ...
            await samo_jedzonko.send(file=discord.File('C:/programowanko/specjal/papanani.jpg'))
            time.sleep(1)
            await samo_jedzonko.send('OOO Papierzu Boże Imperatorze przybądź !!!!')
            time.sleep(3)
            await samo_jedzonko.send(file=discord.File('C:/programowanko/specjal/papemperor1.jpg'))
